...
- Active entrypoint: `src/hbl_etl_dagster/defs_debug.py`
- `src/hbl_etl_dagster/defs.py` is only a compatibility re-export (`from .defs_debug import defs`).
...
Rule of thumb: for production DAG behavior, implement in the active `assets_*` **subfolders** listed above and wire through `defs_debug.py`.
...
4. Prefer explicit imports in `defs_debug.py`; avoid wildcard imports.
...
- IO manager: DuckDB-backed (`duckdb_io_manager`) with DB path currently configured in `defs_debug.py`.
