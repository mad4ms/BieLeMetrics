...
| Job | Purpose |
|-----|---------|
| `season_raw_refresh_job` | Fetch season-level data (competition, fixtures, teams, sessions) |
| `fixture_raw_backfill_job` | Full fixture pipeline: raw → normalized → synced → features → ML |

---

## Known Issues / Tech Debt

- `ml_xg_model` and `ml_xs_model` are inside `fixture_raw_backfill_job` (partitioned), but model
  training is semantically global — fix planned as part of the spatiotemporal upgrade.
...
`debug_run.py` at the repo root runs jobs in-process for fast iteration:

```bash
uv run python debug_run.py season                            # run season_raw_refresh_job
uv run python debug_run.py fixture                           # random fixture (seed=0)
uv run python debug_run.py fixture --seed 3                  # different random fixture
uv run python debug_run.py fixture <fixture_id>              # specific fixture
uv run python debug_run.py fixture <fixture_id> --from shot_events  # from asset onward
uv run python debug_run.py list-fixtures                     # list all known fixture_ids
```

- Prefer `debug_run.py` over `dagster dev` when iterating on pipeline logic.
- Use `--from <asset_name>` to skip expensive upstream steps and re-run only changed assets.
...
Current branch: `fix/partition-aware-io-manager` — fixes to `DuckDBIOManager` for
partition-aware reads/writes, removing manual partition workarounds from asset files and fetchers.
...
| [docs/ml-model.md](docs/ml-model.md) | xG model design, features, training protocol, known limitations |
...
| [docs/timing-async-analysis.md](docs/timing-async-analysis.md) | Kinexon vs Sportradar timing root-cause analysis |
| [docs/shot-sync-drift-findings.md](docs/shot-sync-drift-findings.md) | Per-fixture clock drift investigation; proposed fix |
| [docs/best-practices-pipeline.md](docs/best-practices-pipeline.md) | Adding pipeline functions; data conventions |
| [docs/best-practices-dagster.md](docs/best-practices-dagster.md) | Asset patterns; IO manager selection; job structure |
| [docs/best-practices-testing.md](docs/best-practices-testing.md) | Test structure; sample data; integration tests |
