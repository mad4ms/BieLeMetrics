...
- Fetches detected events via Kinexon get_events_for_session
...
def run_kinexon_positions_and_events_etl(
    ...
    # 2) Detected events
    df_kinexon_events_detected = ingest_detected_events(con, api)
    ...
    n_ev = (
        len(df_kinexon_events_detected)
        if isinstance(df_kinexon_events_detected, pd.DataFrame)
        else 0
    )
    n_en = (
        con.execute(
        ...
        if not df_kinexon_events_detected.empty
        else 0
    )
